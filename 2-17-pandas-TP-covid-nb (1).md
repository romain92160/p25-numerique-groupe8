...
```{code-cell} ipython3
# votre code
france = pd.DataFrame(by_country['France'])
```
...
```{code-cell} ipython3
# votre code
italy = pd.DataFrame(by_country['Italy'])
italy['country'] = 'Italy'
france_italy = pd.concat([france, italy])
```
...
toutpays = [] #liste des dataframe
for nom_pays, info in by_country.items():
    pays = pd.DataFrame(info)
    pays['country'] = nom_pays
    toutpays.append(pays)

total = pd.concat(toutpays)
total.tail()
```
...
```{code-cell} ipython3
# votre code
clean_df = total.set_index(['country', 'date'])
clean_df.head()
```
...
```{code-cell} ipython3
# votre code
francejanvier = clean_df.loc[('France', '2021-01-01')]
francejanvier
```
...
```{code-cell} ipython3
# votre code
francetout = clean_df.loc['France']
francetout
```
...
```{code-cell} ipython3
# votre code
franceitalie = clean_df.loc[(['France', 'Italy'])]
franceitalie
```
...
```{code-cell} ipython3
france[['confirmed', 'deaths', 'recovered']].plot()
plt.show()
```
...
informations = ['deaths', 'confirmed']
df3 = clean_df.loc[(pays, slice(None)), informations].unstack(0)
df3.plot()
plt.show()
```
...
# votre code
def extract(pays, informations, debut, fin):
    df4 = clean_df.loc[(pays, slice(debut, fin)), informations].unstack(0)
    return df4
```
...
```{code-cell} ipython3
# votre code
test = extract(['Afghanistan', 'Zimbabwe'], ['deaths', 'confirmed'],
