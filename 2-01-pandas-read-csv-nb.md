...
```{code-cell} ipython3
# votre code
pd.read_csv('data/petit-titanic.csv')
```
...
```{code-cell} ipython3
colonnes = ['Identifiant', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
dff = pd.read_csv('data/petit-titanic.csv', sep=';', header=None, names=colonnes)
dff.head(2)
```
...
```{code-cell} ipython3
# votre code
#j'ai deja mis celles là mais je remplace Identifiant par PassengerId pour changer: 
columns = ['PassengerId', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
dff.columns = columns
print(dff.columns)
...
```{code-cell} ipython3
# votre code
dff = dff.set_index('PassengerId', inplace = True)
pd.read_csv(dff)
#devrait fonctionner je ne comprends pas 
```
