...
```{code-cell} ipython3
# your code
prev_end = events['end'].shift(1)
overlap = events['beg'] < prev_end
overlap.any()
```
...
```{code-cell} ipython3
# your code
min_beg = events['beg'].min()
max_end = events['end'].max()
timespan = max_end - min_beg
timespan
```
...
overall = events['duration'].sum()
ratio = overall/timespan
ratio
```
...
```{code-cell} ipython3
events_deux = events.set_index('beg')

semaine = events_deux['duration'].resample('W').sum()
mois = events_deux['duration'].resample('M').sum()
an = events_deux['duration'].resample('Y').sum()


graphe, axes = plt.subplots(3, 1)

semaine.plot(kind='bar',ax=axes[0])
mois.plot(kind='bar',ax=axes[1])
an.plot(kind='bar',ax=axes[2])
...
graphique, axes = plt.subplots(3, 1)
for axe, freq in enumerate(['W', 'M', 'Y']):
    s = conversion[freq]
    ax = axes[axe]
    s.plot(kind='bar', ax=ax)
    
    step = SPACES[freq]
    ticks = range(0, len(s), step)
    ax.set_xticks(ticks)
...
```{code-cell} ipython3
# your code
semaine_heures= semaine.map(convert_timedelta_to_hours)
mois_heures= mois.map(convert_timedelta_to_hours)
an_heures= an.map(convert_timedelta_to_hours)

graphe, axes = plt.subplots(3, 1)

for i, (freq, s) in enumerate([('W', semaine_heures),
                               ('M', mois_heures),
                               ('Y', an_heures)]):
    ax= axes[i]
    s.plot(kind='bar', ax=ax)
...
```{code-cell} ipython3
# your code
reg = events.merge(countries, on='name', how='left')
reg.head()
```
